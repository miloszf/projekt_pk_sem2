...
inline bool in_rect(Point point, Point rect)
{
...
void primitive_draw(struct Primitive* primitive, struct RenderInfo r_info)
{
	...
	if (in_rect(r_info.offset, r_info.buffer_size))
	{
		...
		for (unsigned int i = 0; i < length && in_rect(coord, r_info.buffer_size); i++)
		{
			...
			Color bgnd_color = primitive->color;
			Color fgnd_color = (r_info.buffer[buff_coord]).color | primitive->color;
			pixel.color = (bgnd_color & BACKGROUND) | (fgnd_color & FOREGROUND);
